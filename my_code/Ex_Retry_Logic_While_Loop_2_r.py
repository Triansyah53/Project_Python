...
def wait_and_retry_until_status_is_success(max_wait=20,sleep=1):
    timeout=time.time()+max_wait
    while time.time()<timeout:
        is_success=is_status_success()
        print(f'success status is {is_success}')
        if is_success:
            print("SUCCESS")
            break
        else:
            print(f"Not succes sleep for {sleep}")
            time.sleep(sleep)
    else:
        raise Exception(f'Status didnt succes after {max_wait} sec')
